...
## 8. Claim Boundary, Future Work, and Expected Outcomes
...
第一，完成更大规模的外部泛化验证。当前 clean/stress 表已经足够支持内部机制有效性，但 external fixture 规模偏小，只能作为 harm diagnostic。后续应接入更多数据集、更多 forecasting backbone 和更丰富 horizon，尤其要覆盖 DLinear/PatchTST 之外的模型输出。期望结果不是每个外部表都显著提升，而是明确 HalluGuard 适合哪些预测形态、在哪些场景应 abstain，以及稳定 guard 是否能可靠降低过修正风险。

第二，把实验原型整理成外部可接入的 API。理想接口应输入 forecast table、recent context、可选 validation split，输出 corrected forecast、selected action、risk scores、router confidence 和 correction diagnostics。这样 HalluGuard 可以作为独立 post-processing layer 接到其他 forecasting framework 后面，而不是依赖当前实验 runner 的内部目录结构。
